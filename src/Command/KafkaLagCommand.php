...
{
    protected static $defaultName = 'kafka.customs.lag';

    protected function configure()
    {
        $this
            ->setDescription('Poor data consumption')
            ->setHelp('Poor data consumption...')
            ->addOption(
                'topic',
                't',
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $data = ListOffsetsApi::getInstance()->getListOffsets($topic, $partitions, -1);

        $current = array_sum(array_column($result, 'offset')) + count($result);
        $max = array_sum(array_column($data, 'highWatermark'));

        echo ($max - $current) . PHP_EOL;
        exit(0);
    }
}
